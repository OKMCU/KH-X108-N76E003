...
extern void appCmd_waterchk(char *optarg)
{
    char *opt = NULL;
    char *arg = NULL;
    char *remain = NULL;
    ...
    {
        halWaterChkInit();
        printf("Water check inititialised!\r\n");
    }
    else if(strcmp(opt, "-start") == 0)
    {
        halWaterChkStart();
        printf("Water check started!\r\n");
    }
    else if(strcmp(opt, "-ready") == 0)
    {
        printf("Water check result ");
        if(halWaterChkResultReady() == TRUE)
            printf("READY!");
        else
            printf("NOT READY!");
        printf("\r\n");
